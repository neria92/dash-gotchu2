...
export const updateStatus = async ({ captureId, status, userId, mission }) => {
    ...
    if (status === 'Accepted') {
        const money = (user?.userData?.stats?.money || 0) + (mission?.loot?.money || 0)
        const xp = (user?.userData?.stats?.xp || 0) + (mission?.loot?.xp || 0)
        const gCoins = (user?.userData?.stats?.gCoins || 0) + (mission?.loot?.gCoins || 0)
