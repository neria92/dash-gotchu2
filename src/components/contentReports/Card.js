...
export const Card = ({ mission, capture }) => {
    console.log('mission', mission)
    const type = !!mission ? 'mission' : 'capture'
    const title = mission?.missionData?.missionName || capture?.missionData?.missionName || 'Captura libre'
    const image = mission?.missionData?.media?.images[0]?.url || capture?.evidences?.photos[0]?.url ||capture?.evidences?.videos[0]?.thumbnail
    const objective = mission?.missionData?.missionObjetive || capture?.missionData?.missionObjetive || capture?.evidences?.texts
    return (
        <div className="bg-white p-2 w-80 max-w-3xl sm:w-full sm:p-4 h-auto sm:h-64 rounded-2xl shadow-lg flex flex-col sm:flex-row gap-5 select-none my-5">
            ...
            <img src={image} className="h-52 aspect-square sm:h-full sm:w-72 rounded-xl bg-gray-100 bg-center bg-cover" />
